...
# ── 1. File locations ────────────────────────────────────────────────
variant_path = Path("variant.csv")  # Replace with your path
mut_path     = Path("mutations_with_signatures_and_DP.tsv")  # Replace with your path
out_path     = Path("mutations_variant_complete.tsv")  # Output path
...
# Skip the first 832 metadata/comment lines so that row 833 becomes the header
variant_df = pd.read_csv(variant_path, skiprows=831, low_memory=False)
...
# Get only the selected columns that actually exist in variant_df
available_columns = [c for c in selected_columns if c in variant_df.columns]
...
for cols in match_combinations:
    ...
    if not temp.empty:
        partial_matches.append(temp)
        unmatched = unmatched[~unmatched['_original_index'].isin(temp['_original_index'])]

# Combine
all_matches = pd.concat([perfect_matches] + partial_matches, axis=0)

# ── 5. Restore Original Order and Save ───────────────────────────────
final_df = all_matches.sort_values('_original_index').drop(columns=['_original_index'])
