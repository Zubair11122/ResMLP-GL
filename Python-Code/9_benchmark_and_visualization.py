...
df_real["ResMLP-GL_Score"] = y_prob
df_real["ResMLP-GL_Prediction"] = (y_prob > 0.5).astype(int)
...
predicted_top = (
    df_real[df_real["ResMLP-GL_Prediction"] == 1]["hugo_symbol"]
    .value_counts()
    .head(20)
    .index
    .tolist()
...
mutsig_top    = get_top_genes(df_mutsig,   'q-value', 'Gene', 20, True)  # lower q → more significant


# ─── Venn Diagram 1: ResMLP-GL vs OncodriveFML vs CHASMplus ───────────
def plot_venn_oncodrive():
    plt.figure(figsize=(10, 8))
    venn3(
        [set(predicted_top), set(oncodrive_top), set(chasm_top)],
        set_labels=('ResMLP-GL', 'OncodriveFML', 'CHASMplus')
    )
    plt.title("Top 20 Genes: ResMLP-GL vs OncodriveFML vs CHASMplus", fontsize=14)
    plt.savefig(f"{output_dir}/venn_oncodrive_chasm.png", dpi=1200, bbox_inches='tight')
    plt.close()


# ─── Venn Diagram 2: ResMLP-GL vs MutSigCV vs CHASMplus ────────────────
def plot_venn_mutsig():
    plt.figure(figsize=(10, 8))
    venn3(
        [set(predicted_top), set(mutsig_top), set(chasm_top)],
        set_labels=('ResMLP-GL', 'MutSigCV', 'CHASMplus')
    )
    plt.title("Top 20 Genes: ResMLP-GL vs MutSigCV vs CHASMplus", fontsize=14)
    plt.savefig(f"{output_dir}/venn_mutsig_chasm.png", dpi=1200, bbox_inches='tight')
    plt.close()
...
def plot_cosmic_overlap():
    ...
    bars = plt.bar(
        ['ResMLP-GL', 'OncodriveFML', 'CHASMplus', 'MutSigCV'],
        overlaps,
    ...
    plt.ylim(0, 20)
    plt.savefig(f"{output_dir}/cosmic_overlap.png", dpi=1200, bbox_inches='tight')
    plt.close()
...
def plot_performance_curves():
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/performance_curves.png", dpi=1200, bbox_inches='tight')
    plt.close()
...
def generate_summary():
    ...
    data = {
        'Method': ['ResMLP-GL', 'OncodriveFML', 'CHASMplus', 'MutSigCV'],
        'COSMIC_Overlap': [
            len(set(predicted_top) & cosmic_genes),
            len(set(oncodrive_top) & cosmic_genes),
            len(set(chasm_top)     & cosmic_genes),
